...
function GTable(tableDom) {
     ...
     * 加载指定的页数据。
     */
    this._turnToPage = function(pageNo) {
        var rowIndex = this.fspParameter.pagination.rowsPerPage * pageNo;
        var service = ServiceFactory.getService('demo.TableService');
        fspParameter.setStartIndex(rowIndex);
        service.setFspParameter(fspParameter);
...
jQuery.extend(TableBuilder.prototype, {
    ...
    createRow : function (row, rowIndex) {
        ...
        // 构建选择行
        if (this.isCreateSelectionHeader()) {
            var rowHeader = jQuery("<td> <input type='radio' id=" + '_selector_' + id + "  /> </td>");
            tr.append(rowHeader);
        }
...
var TemplateTableConfig = {
     ...
     * 创建一个默认的模板配置
     */
    create : function (className, full) {
    ...
    removeRow : function (event) {
        ...
        if (rowIndex >= 0) {
            ...
            if (removeAction != null) {
                // 调用用户自定义的删除方法
                success = deleteAction(table, selectedRow, service);
            } else {
                // 使用标准的删除方法
